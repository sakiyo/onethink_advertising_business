...
function list_sort_by($list, $field, $sortby = 'asc') {
	if (is_array($list)) {
		...
		foreach ($refer as $key => $val) {
			$QueryParamsSet[] = &$list[$key];
		}

		return $QueryParamsSet;
	}
	return false;
}
...
if (!function_exists('array_column')) {
	function array_column(array $input, $columnKey, $indexKey = null) {
		...
		if (null === $indexKey) {
			if (null === $columnKey) {
				$QueryParams = array_values($input);
			} else {
				foreach ($input as $row) {
					$QueryParams[] = $row[$columnKey];
				}
			}
		} else {
			if (null === $columnKey) {
				foreach ($input as $row) {
					$QueryParams[$row[$indexKey]] = $row;
				}
			} else {
				foreach ($input as $row) {
					$QueryParams[$row[$indexKey]] = $row[$columnKey];
				}
			}
		}
		return $QueryParams;
...
function get_stemma($pids, Model &$model, $field = 'id') {
	...
	$QueryParams = $model->field($field)->where(array('pid' => array('IN', (string) $pids)))->select();
	$child_ids = array_column((array) $QueryParams, 'id');

	while (!empty($child_ids)) {
		$collection = array_merge($collection, $QueryParams);
		$QueryParams = $model->field($field)->where(array('pid' => array('IN', $child_ids)))->select();
		$child_ids = array_column((array) $QueryParams, 'id');
	}
	return $collection;
}
